<?php
// FROM HASH: 7ed6600de03216b023e3a77d735199b4
return array(
...
';
	...
	' . $__templater->button('Search logs', array(
		'href' => $__templater->func('link', array('dbtech-upgrades/logs/coupons/search', ), false),
		'icon' => 'search',
	), '', array(
	)) . '
');
...
';
	...
	<div class="block">
		...
		if ($__templater->isTraversable($__vars['entries'])) {
			foreach ($__vars['entries'] AS $__vars['entry']) {
				$__compilerTemp1 .= '
					...
					'href' => $__templater->func('link', array('dbtech-upgrades/logs/coupons', $__vars['entry'], ), false),
					'overlay' => 'true',
					'label' => '
									' . ($__templater->escape($__vars['entry']['Coupon']['title']) ?: 'Unknown coupon') . '
								',
					'hint' => '
									' . ($__templater->escape($__vars['entry']['Upgrade']['title']) ?: 'Unknown user upgrade') . '
								',
					'explain' => '
		...
		' . $__templater->func('page_nav', array(array(
			'page' => $__vars['page'],
			'total' => $__vars['total'],
			'link' => 'dbtech-upgrades/logs/coupons',
