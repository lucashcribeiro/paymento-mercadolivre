...
';
	...
	if (!$__templater->test($__vars['available'], 'empty', array())) {
		$__compilerTemp1 .= '
			' . $__templater->includeTemplate('dbtech_user_upgrade_coupon', $__vars) . '
			<div class="block">
		...
		if ($__templater->isTraversable($__vars['available'])) {
			foreach ($__vars['available'] AS $__vars['upgrade']) {
				$__compilerTemp1 .= '
						';
				$__compilerTemp2 = '';
				if (((!$__templater->method($__vars['upgrade'], 'isFreeAfterCoupon', array())) AND ($__templater->func('count', array($__vars['upgrade']['payment_profile_ids'], ), false) > 1))) {
					$__compilerTemp2 .= '
										';
					...
					$__compilerTemp2 .= $__templater->formSelect(array(
						'name' => 'payment_profile_id',
										...
										';
					if ($__templater->method($__vars['upgrade'], 'isFreeAfterCoupon', array())) {
						$__compilerTemp2 .= '
												' . $__templater->button('Get free', array(
							'type' => 'submit',
						), '', array(
						)) . '
											';
				...
				$__compilerTemp1 .= $__templater->formRow('
							...
							', array(
					'action' => $__templater->func('link', array('purchase' . ($__templater->method($__vars['upgrade'], 'isFreeAfterCoupon', array()) ? '/free-user-upgrade' : ''), $__vars['upgrade'], array('user_upgrade_id' => $__vars['upgrade']['user_upgrade_id'], ), ), false),
					'ajax' => 'true',
...
';
					if ($__templater->method($__vars['upgrade'], 'canRenew', array())) {
						$__compilerTemp4 .= '
	' . $__templater->button('
		' . $__templater->fontAwesome('fas fa-sync', array(
						)) . ' ' . 'Renew' . '
	', array(
							'href' => $__templater->func('link', array('renew-sub/renew', $__vars['upgrade'], ), false),
							'overlay' => 'true',
						), '', array(
						)) . '
';
