...
class Pdf
{
	 ...
	 * @var mixed
	 */
	protected $flt_width_points;
	protected $flt_height_points = 0.00;
	 ...
	 * @var mixed
	 */
	protected $flt_current_width;
	protected $flt_current_height = 0.00;
	 ...
	 * @var
	 */
	protected $flt_position_x;
	protected $flt_position_y = 0.00;
	...
	public function __construct($str_orientation = 'P', $str_units = 'mm', $str_size = 'A4')
	{
		...
		elseif (getenv('FPDF_FONT_WRITE_PATH'))
		{
	 ...
	 * @param $str_message
	 */
	protected function Error($str_message)
	{
		// Fatal error
	...
	public function Line($flt_x_1, $flt_y_1, $flt_x_2, $flt_y_2)
	{
		...
		$this->Out(sprintf(
			'%.2F %.2F m %.2F %.2F l S',
			$flt_x_1 * $this->flt_scale_factor,
			($this->flt_current_height - $flt_y_1) * $this->flt_scale_factor,
			$flt_x_2 * $this->flt_scale_factor,
	...
	public function Rect($flt_x, $flt_y, $flt_width, $flt_height, $str_style = '')
	{
		...
		$this->Out(sprintf(
			...
			($this->flt_current_height - $flt_y) * $this->flt_scale_factor,
	...
	public function AddFont($str_family, $str_style = '', $str_file = '', $bol_unicode = false)
	{
		...
		if ($bol_unicode)
		{
			...
			$this->arr_font_files[$fontkey] = ['length1' => $originalsize, 'type' => self::FONT_TRUETYPE, 'ttffile' => $str_ttf_filename];
			$this->arr_font_files[$str_file] = ['type' => self::FONT_TRUETYPE];
		...
		else
		{
			...
			if (!empty($arr_info['file']))
			{
				...
				{
					$this->arr_font_files[$arr_info['file']] = ['length1' => $arr_info['originalsize']];
				}
				else
				{
					$this->arr_font_files[$arr_info['file']] = [
	...
	public function AddLink()
	{
		...
		$this->arr_internal_links[$int_count] = [0, 0];
		return $int_count;
	}
	...
	public function SetLink($mix_link_key, $int_y = 0, $int_page = -1)
	{
		...
		$this->arr_internal_links[$mix_link_key] = [$int_page, $int_y];
	}
	...
	public function Link($flt_x, $flt_y, $flt_width, $flt_height, $mix_link_key)
	{
		...
		$this->arr_page_links[$this->int_page][] = [
			$flt_x * $this->flt_scale_factor,
	...
	public function Cell($flt_width, $flt_height = 0, $str_text = '', $int_border = 0, $int_line_number = 0, $str_alignment = '', $bol_fill = false, $str_link = '')
	{
		...
		if ($bol_fill || $int_border == 1)
		{
			...
			$str_output = sprintf(
				'%.2F %.2F %.2F %.2F re %s ',
				$this->flt_position_x * $flt_scale,
				($this->flt_current_height - $this->flt_position_y) * $flt_scale,
				$flt_width * $flt_scale,
				-$flt_height * $flt_scale,
				$str_operation
			);
		}
		if (is_string($int_border))
		{
			...
			if (strpos($int_border, 'L') !== false)
			{
				$str_output .= sprintf(
					'%.2F %.2F m %.2F %.2F l S ',
					$flt_position_x * $flt_scale,
					($this->flt_current_height - $flt_position_y) * $flt_scale,
					$flt_position_x * $flt_scale,
			...
			if (strpos($int_border, 'T') !== false)
			{
				$str_output .= sprintf(
					...
					$flt_position_x * $flt_scale,
					($this->flt_current_height - $flt_position_y) * $flt_scale,
					($flt_position_x + $flt_width) * $flt_scale,
					($this->flt_current_height - $flt_position_y) * $flt_scale
				);
			}
		...
		if ($str_text !== '')
		{
			...
			if ($this->int_word_spacing && $this->bol_uniform_subset)
			{
				...
				$str_output .= sprintf(
					'BT 0 Tw %.2F %.2F Td [',
					($this->flt_position_x + $flt_dimensions) * $flt_scale,
			...
			{
				$str_output .= ' ' . $this->DoUnderline(
					$this->flt_position_x + $flt_dimensions,
					$this->flt_position_y + .5 * $flt_height + .3 * $this->int_font_size_user,
					$str_text
				);
			}
			if ($this->bol_fill_text_differ)
			...
			if ($str_link)
			{
				$this->Link(
					$this->flt_position_x + $flt_dimensions,
					$this->flt_position_y + .5 * $flt_height - .5 * $this->int_font_size_user,
					$this->GetStringWidth($str_text),
					$this->int_font_size_user,
					$str_link
				);
			}
		}
	...
	public function Image($str_file, $flt_x = null, $flt_y = null, $int_width = 0, $int_height = 0, $str_type = '', $str_link = '')
	{
		...
		$this->Out(sprintf(
			...
			$int_width * $this->flt_scale_factor,
			$int_height * $this->flt_scale_factor,
			$flt_x * $this->flt_scale_factor,
			($this->flt_current_height - ($flt_y + $int_height)) * $this->flt_scale_factor,
			$arr_image_info['i']
		));
		if ($str_link)
		{
	 ...
	 * @return array|string
	 */
	protected function getPageSize($mix_size)
	{
		if (is_string($mix_size))
		{
			...
			return [$a[0] / $this->flt_scale_factor, $a[1] / $this->flt_scale_factor];
		}
		else
		{
			if ($mix_size[0] > $mix_size[1])
			{
				return [$mix_size[1], $mix_size[0]];
			}
			else
			{
				return $mix_size;
	...
	protected function BeginPage($str_orientation, $mix_size)
	{
		...
		{
			$this->arr_page_sizes[$this->int_page] = [$this->flt_width_points, $this->flt_height_points];
		}
	}
	 ...
	 * @return array
	 */
	protected function LoadFont($str_font)
	{
	 ...
	 * @return string
	 */
	protected function UTF8toUTF16($str_text)
	{
	...
	protected function DoUnderline($flt_x, $flt_y, $str_text)
	{
		...
		return sprintf(
			...
			$flt_width * $this->flt_scale_factor,
			-$flt_underline_thickness / 1000 * $this->int_current_font_size
		);
	}
	 ...
	 * @return array
	 */
	protected function _parsejpg($str_file)
	{
		...
		return ['w' => $arr_size_data[0], 'h' => $arr_size_data[1], 'cs' => $str_color_space, 'bpc' => $int_bits_per_component, 'f' => 'DCTDecode', 'data' => $str_data];
	}
	 ...
	 * @return array
	 */
	protected function _parsepng($str_file)
	{
	 ...
	 * @return array
	 */
	protected function _parsepngstream($ptr_file, $str_file)
	{
		// Check signature
		...
		do
		{
			...
			elseif ($str_type == 'tRNS')
			{
				...
				{
					$arr_transparency_data = [ord(substr($str_transparency, 1, 1))];
				}
				elseif ($int_color_channels == 2)
				{
					$arr_transparency_data = [ord(substr($str_transparency, 1, 1)), ord(substr($str_transparency, 3, 1)), ord(substr($str_transparency, 5, 1))];
				}
				else
				{
	 ...
	 * @return string
	 */
	protected function _readstream($ptr_file, $int_bytes)
	{
	 ...
	 * @return mixed
	 */
	protected function _readint($ptr_file)
	{
	 ...
	 * @return array
	 */
	protected function _parsegif($str_file)
	{
	...
	public function PutTTFontWidths(&$font, $maxUni)
	{
	...
	{
		$arr_output = [];
		$splitStr = str_split($str_input);
		$int_string_length = count($splitStr);
		for ($i = 0; $i < $int_string_length; $i++)
		{
			$uni = -1;
			$int_char = ord($splitStr[$i]);
			if ($int_char <= 0x7F)
			{
				$uni = $int_char;
			}
			elseif ($int_char >= 0xC2)
			{
				...
				{
					$uni = ($int_char & 0x1F) << 6 | (ord($splitStr[++$i]) & 0x3F);
				}
				elseif (($int_char <= 0xEF) && ($i < $int_string_length - 2))
				{
					$uni = ($int_char & 0x0F) << 12 | (ord($splitStr[++$i]) & 0x3F) << 6
						| (ord($splitStr[++$i]) & 0x3F);
				}
				elseif (($int_char <= 0xF4) && ($i < $int_string_length - 3))
				{
					$uni = ($int_char & 0x0F) << 18 | (ord($splitStr[++$i]) & 0x3F) << 12
						| (ord($splitStr[++$i]) & 0x3F) << 6
						| (ord($splitStr[++$i]) & 0x3F);
				}
			}
			if ($uni >= 0)
